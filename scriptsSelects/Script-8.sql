drop database if exists extrascolars;    -- eliminar la base de dades
create database if not exists extrascolars;  -- crear la base de dades

use extrascolars;

create table escoles(
	id_escola int primary key,
	nom_escola varchar(30) not null
);

insert into escoles values (1,'IES Sabadell'),
						   (2,'IES Terrassa'),
						   (3,'IES Badia'),
						   (4,'IES Estatut');

select * from escoles;

create table poblacions (
	id_poblacio char(3) primary key,
	nom_poblacio varchar(30) not null unique,
	habitants int);

insert into poblacions (id_poblacio, nom_poblacio) values
	('SAB', 'Sabadell'),
	('TER', 'Terrassa'),
	('SQV', 'Sant Quirze del Vallès'),
	('BAD', 'Badia'),
	('RUB', 'Rubi'),
	('CER', 'Cerdanyola'),
	('SCG', 'Sant Cugat');
	
select * from poblacions;


-- volem posar que 'Sabadell' te 210000 habitants
-- volem posar que 'Terrassa' te 209999 habitants

update poblacions set habitants = 210000 
where id_poblacio = 'SAB';

update poblacions set habitants = 209999 
where id_poblacio = (select id_poblacio
					 from poblacions
					 where nom_poblacio = 'Terrassa');

update poblacions set habitants = 209999 
where nom_poblacio = 'Terrassa';

/*
CRUD  
								SQL
C   create	crear		**** create 
R	read	llegir		**** select  ***
U	update	actualitzar **** update
D	delete	esborrar	**** delete
*/

-- volem eliminar Sant Cugat

delete from poblacions where nom_poblacio = 'Sant Cugat';

-- afegir un camp a una taula
alter table escoles add column id_poblacio char(3);

-- fer que un camp sigui foreign key
alter table escoles add constraint foreign key (id_poblacio) 
references poblacions(id_poblacio);

select * from escoles;

-- afegem dues escoles de Sabadell

insert into escoles values (5,'Escola Industrial','SAB');

select max(id_escola) from escoles;

insert into escoles values (
	(select ifnull(max(id_escola),0) from escoles)+1, 'IES Valles','SAB');	

-- poso que IES Sabadell és de Sabadell (versió barata)
update escoles set id_poblacio = 'SAB' 
where nom_escola = 'IES Sabadell';

-- les següents sentències fallen, 
-- per les regles d'integritat referencial
-- puc dir que 'IES Estatut' és de Mataro?  ('MAT')
-- si no tens creada la població 'MAT' no podràs

-- update escoles set id_poblacio = 'MAT' 
-- where nom_escola = 'IES Estatut';

-- puc esborrar la població 'Sabadell'  (no, fallarà)
-- delete from poblacions where nom_poblacio = 'Sabadell';

create table nens (
	idNen int auto_increment primary key,
	nom_nen varchar(30) not null,
	cognoms_nen varchar (30) not null,
	telefon varchar(15),
	id_poblacio char(3),
	id_escola int,
	foreign key (id_poblacio) references poblacions(id_poblacio),
	foreign key (id_escola) references escoles(id_escola)
);

insert into poblacions values ('CDV','Castellar del Valles',null),
							  ('BDV', 'Barberà del Vallès', null),
							  ('VSS', 'Vilalba Sasserra', null);

delete from nens;

insert into nens (nom_nen, cognoms_nen, id_poblacio, id_escola) values
	('Sergio','Sanchez','BDV',1),
	('Ferran', 'Vidal', 'SAB',1),
	('Isaac', 'Saez', 'SAB', 2),
	('Paula', 'Hernandez','BAD',2),
	('Ismael','Guzman', 'CDV',2),
	('Montserrat','Villanueva','VSS',2),
	('Jose','Campos','SAB',1),
	('Juan Manuel', 'Herrera', 'CDV', 2);

select * from nens;

-- enviar a tothom que no sigui de 'Sabadell' a l'IES Terrassa

update nens set id_escola = (select id_escola 
							 from escoles 
							 where nom_escola = 'IES Terrassa')
where id_poblacio != (select id_poblacio
					  from poblacions
					  where nom_poblacio = 'Sabadell');

select * from nens;

create table activitats (
	id_activitat int auto_increment primary key,
	nom_activitat varchar(30) not null 
);

insert into activitats (nom_activitat) values ('escacs'),
											  ('petanca'),
											  ('punto y confección'),
											  ('jabalina'),
											  ('tiro al plato');

create or replace table escoles_activitats (
	id_escola int,
	id_activitat int,
	punts int default 5,
	places int default 20,
	primary key (id_escola, id_activitat),
	foreign key (id_escola) references escoles(id_escola),
	foreign key (id_activitat) references activitats (id_activitat)
);

-- vull que en totes les escoles s'ofereixi escacs
/*
insert into escoles_activitats (id_escola, id_activitat)
 (select ...);
*/
select id_activitat 
from activitats 
where nom_activitat = 'escacs'

describe escoles_activitats;

select id_escola,(select id_activitat 
			      from activitats 
			      where nom_activitat = 'escacs')  
from escoles;

insert into escoles_activitats (id_escola, id_activitat)
		(select id_escola,(select id_activitat 
			      from activitats 
			      where nom_activitat = 'escacs')  
		from escoles);


-- digues les escoles de 'Sabadell' on s'ofereix 'escacs'
